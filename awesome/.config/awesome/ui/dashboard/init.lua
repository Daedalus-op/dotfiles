...
awful.screen.connect_for_each_screen(function(s)
    ...
    local width = beautiful.dash_width
    ...
    s.dashboard.popup = wibox({
        ...
        width = 1,
        height = s.geometry.height - beautiful.useless_gap * 4 - beautiful.bar_height,
        x = s.geometry.x + beautiful.useless_gap * 2,
        y = s.geometry.y + beautiful.useless_gap * 2 + beautiful.bar_height,
    })
    ...
    self.animate:subscribe(function(animfactor)
        ...
        if self.status ~= "undefined" then
            ...
            if self.status == "closing" and animfactor < 1 then
                hide()
            elseif self.status == "closing" and not self.animate.running then
                hide()
            end
        end
    end)
