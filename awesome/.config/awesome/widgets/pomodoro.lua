...
local NOTIFY_pomodoro_CMD = "pomo notify"
...
local update_graphic = function(widget, stdout, _, _, _)
	...
	if pomostatus == "--" then
		...
		text.font = "Inconsolata Medium 8"
		...
		if status == " " then -- clock ticking
			if workbreak == "W" then
				...
				if tonumber(pomomin) < 5 then -- last 5 min of pomo
					widget.colors = { beautiful.pomo_5_ }
				else
					widget.colors = { beautiful.pomo_W_ }
				end
			elseif workbreak == "B" then -- color during pause
				widget.colors = { beautiful.pomo_B_ }
				widget.value = tonumber(pomodoro / (5 * 60))
			end
		elseif status == "P" then -- paused
			if workbreak == "W" then
				widget.colors = { beautiful.pomo_WP }
				widget.value = tonumber(pomodoro / (25 * 60))
				text.font = "Inconsolata Medium 8"
				text.text = pomomin -- "PW"
			elseif workbreak == "B" then
				widget.colors = { beautiful.pomo_BP }
				widget.value = tonumber(pomodoro / (5 * 60))
				text.font = "Inconsolata Medium 8"
				text.text = pomomin -- "PB"
			end
		end
	end
end
...
awful.spawn(NOTIFY_pomodoro_CMD, false)
local notification
