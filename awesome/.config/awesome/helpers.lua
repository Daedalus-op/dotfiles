			...
			{
				{
					{
						{
							markup = helpers.complex_capitalizing(n.app_name == "" and "Misc" or n.app_name),
							align = "left",
							valign = "center",
							widget = wibox.widget.textbox,
						},
...
local function notify_card_body(n, layout)
	...
	layout:add(wibox.widget({
		markup = n.title == "" and "<b>" .. helpers.limit_by_length(n.message, 23, true) .. "</b>"
			or helpers.limit_by_length(n.message, 36, true),
		...
		{
			{
				n.icon and {
					...
					markup = helpers.get_colorized_markup("󰋽", beautiful.blue),
	...
	return {
		{
			{
				{
					{
						{
							markup = helpers.complex_capitalizing(n.app_name == "" and "App" or n.app_name),
			...
			n.app_name == "Screenshot" and not not n.icon and helpers.get_sidebar_screenshot_body(n)
